...
export default function ResultsPage({
    ...
    const { data, error, isLoading } = useSWR<Results>(`/api/session/results?apiKey=${apiKey}&questionnaireId=${qId}&sessionId=${session?.id}`, fetcher)
    ...
    return (
        <div className="w-full flex flex-col justify-center items-center">
            <div className="flex flex-col justify-center items-center">
                ...
                <div className='pt-5'>
                    <StyledButton value="Continuer" action={() => router.push(`/${apiKey}/questionnaires`)} />
                </div>
            </div>
        </div>
    );
