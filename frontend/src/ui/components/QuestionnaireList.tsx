...
export function QuestionnaireList({ questionnaires, loading, error }: QuestionnaireListProps) {
    if (loading) {
    ...
    return (
        <div className="flex flex-wrap justify-center items-center gap-5 md:flex-row pb-5">
            {questionnaires.map(q => (
                <h2 key={q.id} className={`text-sm font-${q.isNext ? 'bold' : 'semibold'}`}>{q.name}</h2>
            ))}
        </div>
    );
}