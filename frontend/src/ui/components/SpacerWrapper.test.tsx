...
describe('SpacerWrapper', () => {
    ...
    it('devrait correctement ajouter les spacer entre chaque enfants', () => {
        const { container } = render((
            <SpacerWrapper spacer={<span data-testid="spacer">space-spacer</span>}>
    ...
    it('devrait rendre les enfants dans le bon ordre', () => {
        const { container } = render((
            <SpacerWrapper spacer={<span data-testid="spacer">space-spacer</span>}>
                <div data-testid="child-1"></div><div data-testid="child-2"></div>
            </SpacerWrapper>
        ));

        expect(container.children).toHaveLength(3);
        expect(container.children[0].getAttribute('data-testid')).toBe("child-1");
        expect(container.children[1].getAttribute('data-testid')).toBe("spacer");
        expect(container.children[2].getAttribute('data-testid')).toBe("child-2");
    });
})
